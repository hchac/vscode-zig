...
namespace MIOutputParser {
  ...
  function parseList(miOutput: string, startAt: number): [number, Array<any>] {
    let results: any[] = [];
    let i = startAt;
    while (miOutput[i] != "]") {
      ...
      if (miOutput[i] == ",") {
        ...
        throw new Error("Unknown position after parsing result inside list");
      }
    }
  ...
  function parseTuple(miOutput: string, startAt: number): [number, any] {
    ...
    while (miOutput[i] != "}") {
      ...
      if (miOutput[i] == ",") {
        ...
        throw new Error("Unknown position after parsing result inside tuple");
      }
    }
...
namespace MIOutputVariableParser {
  function parseType(output: string, startAt: number): [number, string] {
    if (output[startAt] != "<")
      throw new Error("Parse type not starting in appropriate place");

    for (
      let curPosition = startAt + 1;
      curPosition < output.length;
      curPosition++
    ) {
      if (output[curPosition] == ">") {
        return [curPosition + 1, output.slice(startAt + 1, curPosition)]; // startAt + 1 to skip "<" and curPosition will avoid ending ">"
      }
    }

    throw new Error(
      `Failed to get the type name from output, starting at: ${startAt}`
    );
  }
  ...
  function parseArray(
    ...
    curPosition += 1; // Skip "}" at end of array

    return [curPosition, { kind: "array", data: arr }];
  }
  ...
  function parseStruct(
    ...
    while (output[curPosition] != "}") {
      if (output[curPosition] != "[")
        throw new Error(
          `Invalid state while parsing struct. Expected start of field name at position: ${curPosition}`
        );

      let endOfFieldName = curPosition;
      while (output[endOfFieldName] != "]") endOfFieldName += 1;
      endOfFieldName += 1; // Skip past ']'

      const fieldName = output.slice(curPosition + 1, endOfFieldName - 1); // +1 to skip '[' and -1 to avoid ']' in  the type name

      curPosition = endOfFieldName + 3; // Skip past " = " to go straight to value

      let value;
      [curPosition, value] = parseOutput(output, curPosition);
      data[fieldName] = value;

      if (output[curPosition] != "}") {
        if (output[curPosition] != ",")
          throw new Error(
            `Expected end of struct or end of current element at position: ${curPosition}`
          );

        curPosition += 2; // Skip ", " to start of next element
      }
    }

    curPosition += 1; // Skip '}' at end of struct
...
interface StackVariable {
  ...
  // to see its insides.
  value: undefined | MIVariableOutput;
}
...
class DebuggerInterface {
  ...
  // Set a callback to be notified when a stop even occurs in gdb
  public stopEventNotifier: (record: MIOutputParser.RecordOutput) => void;
  // Set a callback to be notified when an output event occurs in gdb
  ...
  public async launch(cwd?: string) {
      ...
      for (const line of strData.split("\n")) {
        ...
        try {
          ...
          if (line.indexOf("location added to breakpoint") != -1) {
            // TODO: relevant now that we're using only gdb?
        ...
        if (record.kind == "result-record" && record.token != null) {
          ...
          if (callback) {
            ...
            loge(
              `Have data for mi command with token but no callback is registered: '${record.token}'`
            );
          }
        } else if (
    ...
    const prettyPrinterFile = "./zig_pretty_printer.py";
    try {
      // Lets execute our pretty printer file.
      // Our pretty printer file does extra work in printing out different data types
      // in order to get more information, or just simply make them easier to parse.
      await this.runMiCommand(
        `python execfile("${prettyPrinterFile}")`,
        MIOutputParser.ResultClasses.done
      );
    } catch (err) {
      throw new Error(`Failed to activate custom pretty printers: ${err}`);
    }
  }
    ...
    return await new Promise((res, rej) => {
      const receive = (result: MIOutputParser.RecordOutput) => {
        ...
        if (result.class === expectedResultClass) {
          ...
          return rej(`Unexpected output for runMiCommand with token ${token}`);
        }
      };
  ...
  private async attachRun(config: AttachConfig) {
    ...
    try {
      ...
      throw new Error(
        `Failed to continue execution after attaching ro ${config.processId}: ${err}`
      );
    }
  }
  ...
  public kill() {
    ...
    // TODO: try quitting in (gdb/mi) first
  ...
  public async insertBreakpoint(
    ...
    // If a user specifies a breakpoint at say line 5, and there's no code at
    // that line, gdb will let you know where it actually found a spot to
    // place the breakpoint further down.
    ...
    if (condition) {
      try {
        switch (condition.kind) {
          case "hitCondition": {
            await this.runMiCommand(
              `-break-after ${breakpointId} ${condition.condition}`,
              MIOutputParser.ResultClasses.done
            );
            break;
          }
          case "condition": {
            await this.runMiCommand(
              `-break-condition ${breakpointId} ${condition.condition}`,
              MIOutputParser.ResultClasses.done
            );
            break;
          }
  ...
  private parseStackFrames(threadId: number, frames: any[]) {
    ...
    for (const f of frames) {
      ...
      //     "line": "-1"
      // }
      if (f.line == "-1" || f.file == "??" || f.fullname == "??") continue;
  ...
  public async dataEval(expr: string): Promise<MIVariableOutput> {
    ...
    if (!output || !output.value) {
      throw new Error(
        `Failed to get value output from: ${JSON.stringify(output)}`
      );
    }
    ...
    return new Promise((res, rej) => {
      const receive = (result: MIOutputParser.RecordOutput) => {
        ...
        if (usingMi && result.class == "done") {
          ...
          return rej(
            `Unexpected output from evaluate: ${JSON.stringify(result)}`
          );
        }
      };
...
class ZigDebugSession extends LoggingDebugSession {
  ...
  private async launchOrAttach(
    ...
    if (args.pathToDebugger.indexOf("gdb") == -1) {
      this.sendErrorResponse(
        response,
        ErrorCodes.UnrecognizedDebugger,
        "Could not recognize debugger in pathToDebugger. Please make sure you are using gdb."
      );
      return;
    }
  ...
  private setupNotifiers() {
    this.debgugerInterface.stopEventNotifier = record => {
      ...
      if (record.output.reason == "exited-normally") {
        ...
        if (isNaN(threadId)) {
          loge(
            `Failed to parse thread id from "${record.output["thread-id"]}" on stop event`
          );
        } else {
          const event = new StoppedEvent(record.output.reason, threadId);
  ...
  protected async stackTraceRequest(
    ...
    try {
      ...
      if (!threadInfo) {
        throw new Error(
          `stackTraceRequest with threadId ${args.threadId} that has no corresponding data`
        );
      }

      response.body.stackFrames = threadInfo.frames.map(s => {
        const uniqueId = this.stackFrameHandles.create([s.threadId, s.level]);
        return new StackFrame(
          uniqueId,
          s.function,
          new Source(s.file, s.filePath),
          s.line,
          0 // TODO: can we get column number from gdb?
        );
      });
    } catch (err) {
  ...
  protected async scopesRequest(
    ...
    try {
      const [threadId, frameLevel] = this.stackFrameHandles.get(args.frameId);
  ...
  protected async variablesRequest(
    ...
    if (varHandle.kind === "locals") {
      ...
      // Note that because of --simple-values the output from -stack-list-variables
      // does not include composite datatypes, such as structs, etc. In this branch
      // we will mark those composite variables as "pending-eval" so that they get
      // added to the variable handles set. The user will see these composite variables
      // as just the variable name followed by the type, but they will be able to click
      // on that variable to force us to evaluate and get the actual value of those
      // variables. That is handled by the other branch.
      ...
      let varOutput: MIVariableOutput;
      ...
      switch (varHandle.kind) {
        case "pending-eval": {
          ...
          try {
            ...
            if (isPointer) {
              ...
              varOutput = await this.debgugerInterface.dataEval(fullVarPath);
            } else {
              varOutput = await this.debgugerInterface.dataEval(fullVarPath);
            }
          } catch (err) {
        ...
        case "inner-value": {
          // This has already been evaluated, lets just return the returned value.
  ...
  protected async evaluateRequest(
    ...
    try {
      if (args.context == "repl") {
        const result = await this.debgugerInterface.evaluate(args.expression);
        response.body = {
          result,
        ...
        const result = await this.debgugerInterface.dataEval(args.expression);
  ...
  protected async setVariableRequest(
    ...
    try {
      ...
      loge(
        `Failed to set "${fullVariablePath}" with value <${args.value}>: ${err}`
      );
    }
